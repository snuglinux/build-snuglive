...
iso_publisher="SnugLinux <https://snuglinux.pp.ua>"
iso_application="SnugLinux Live/Rescue CD"
...
make_basefs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode amd-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh efitools" install
}
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_efi() {
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
        ...
        ${script_path}/efiboot/loader/entries/archiso-${arch}-usb.conf > ${work_dir}/iso/loader/entries/archiso-${arch}.conf
...
make_efiboot() {
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-${arch}-cd.conf > ${work_dir}/efiboot/loader/entries/archiso-${arch}.conf
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -P "${iso_publisher}" -A "${iso_application}" -o "${out_dir}" iso "${iso_name}-${iso_version}-${arch}.iso"
}

# Перед каждым запуском нужно очищать директорию work
clear_work() {
   cur_dir=$PWD
   if [ -d "${cur_dir}/${work_dir}/efiboot" ]; then
      if [[ `mount | grep ${cur_dir}/${work_dir}/efiboot` != "" ]]; then
          umount -d "${cur_dir}/${work_dir}/efiboot"
      fi
   fi
   if [ -d "${cur_dir}/${work_dir}" ]; then
      rm -r ${cur_dir}/${work_dir}
   fi
}

# проверка существования пакетов в репозиториях
check_package_existence() {
  pkg_list=$1
  echo "Package checking in progress"
  if [ -z "$pkg_list" ]; then
      echo "Empty list of packages to check"
     return 1;
  fi
  for line in $pkg_list; do
     local name_pkg=`pacman -Ssq $line | grep -x $line`
     if [ -z "$name_pkg" ]; then
        echo "Error: not found package '$line' in repositories!"
        return 1;
     fi
  done
  return 0;
...
set +e
INST_ARCHISO=`pacman -Qs archiso`
if [ -z "$INST_ARCHISO" ]; then
...
# проверим наличие пакетов в репозиториях
check_package_existence "$(grep -h -v ^# ${script_path}/packages.${arch}) haveged intel-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh efitools"
if [ "$?" = 1 ]; then
   exit 1;
fi
